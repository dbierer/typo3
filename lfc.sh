#!/bin/bash
TOOLS_DIR=~/Repos/phpcl/lfc_tools
LFC_DIR=~/Repos/phpcl/linuxforcomposer/bin
LFC_PID=`pwd`/vendor/composer
export URL=`php -r "echo basename(__DIR__);"`
export CONTAINER=`echo $URL |sed 's/\./_/g'`
export USAGE="Usage: lfc.sh up|down|start|stop|php|deploy|init|shell|swap|creds"
export INIT=0
export SWAP=0
